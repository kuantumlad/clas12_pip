...
class MLEPionMinusBeta implements BIParticleCandidate{
	...
	double conf_lvl = 0.0;
	    ...
	    LorentzVector lv_pp = Calculator.lv_particle(bev.recBank, rec_i, PhysicalConstants.pionplusID); //SIGN DOESNT MATTER FOR LVector
	    double p_pp = lv_pp.p();
		...
		if( scint_detector == 12 ){
		    ...
		    if( scint_layer == 2 ){
			likelihood = PionPlusPDF.pionMinusBetaMLE(beta, beta_hypoth, p_pp, sector); //hypoth is the "true" beta
			conf_lvl = PionPlusPDF.pionMinusBetaConfLevel(beta, beta_hypoth, p_pp, sector); //hypoth is the "true" beta
		    }
		    /*else if( scint_layer == 1 ){
			likelihood = PionPlusPDF.pionMinusBetaMLE(beta, beta_hypoth, p_pp, sector); // same as above comment
			conf_lvl = PionPlusPDF.pionMinusBetaConfLevel(beta, beta_hypoth, p_pp, sector); //hypoth is the "true" beta
		    }
		    */			    
		}
