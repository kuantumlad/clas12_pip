...
public class BResKaonPlus {

    int run_number = -1;
    String s_run = "";
    public BResKaonPlus( int temp_run ){
	run_number = temp_run;
    ...
    H2F h2_rc_kp_betap;
    H1F h_rc_kp_beta_res;


    public void createResKaonPlusHistograms(){

 	////////////////////////////////////////////////////////////////////
	//BETA STUDY
	h_rc_kp_beta_res = new H1F("h_rc_kp_beta_res","h_rc_kp_beta_res", 200, -0.05, 0.05 );
	h2_rc_kp_betap = new H2F("h2_rc_kp_betap","h2_rc_kp_betap", 100, 0.4, 5.5, 100, -0.04, 0.04);
 
    }

    public void saveResKaonPlus(){


    }

    public void sliceNFitResKaonPlus() {
	...
	H2F h2_temp_rebinX = h2_rc_kp_betap.rebinY(5);
	H2F h2_temp_rebinXY = h2_rc_kp_betap.rebinX(5);
	...
	g_mean.setTitle(" MEAN BETA FOR KAON PLUS  ");
	g_sigmas.setTitle(" MEAN BETA FOR KAON PLUS ");

	for( int n_htemp = 2; n_htemp < h_temp_rebinXY_sliceX.size() - 1; n_htemp++ ){
	    c_slices.cd(n_htemp);
	...
	c_slices.save("/lustre/expphy/work/hallb/clas12/bclary/pics/pid_clary/h_kp_resbeta_slices_allsector.png"); 
	c_betap_fit.save("/lustre/expphy/work/hallb/clas12/bclary/pics/pid_clary/g_kp_resbeta_fit_allsector.png"); 


    }


    public void histoResKaonPlusToHipo(){
	
	dir.mkdir("/res_pos_betap/");
	dir.cd("/res_pos_betap/");
	dir.addDataSet(h2_rc_kp_betap);

	dir.addDataSet(h_rc_kp_beta_res);
	EmbeddedCanvas c_kp_res = new EmbeddedCanvas();
	c_kp_res.setSize(900,900);
	System.out.println(" >> FITTING BETA RESOLUTION HISTOGRAM " );
	F1D f_beta_res = Calculator.fitHistogram(h_rc_kp_beta_res,0.6);
	...
	c_kp_res.draw(h_rc_kp_beta_res,"same");
	c_kp_res.draw(f_beta_res,"same");
	c_kp_res.save("/lustre/expphy/work/hallb/clas12/bclary/pics/pid_clary/h_rc_kp_beta_res_allsector.png");

    }

    public void viewResKaonPlusHipoOut(){
