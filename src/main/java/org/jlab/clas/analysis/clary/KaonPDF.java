...
class KaonPDF{
    ...
    public static double kaonMinusBetaMLE( double beta_meas, double beta_true, double p, int sector ){
	///////meas is using tof and path length while beta_true is from the mass and mntm of the particle
	...
	if( sector == 0 ){
	    min_beta = CutLoader.getMinKMBetaSector1Cut(); // CONTAINS MEAN VALUES HUGE TYPE 
	    max_beta = CutLoader.getMaxKMBetaSector1Cut(); // CONTAINS SIGMA VALUES 
	}
	else if( sector == 1 ){
	    min_beta = CutLoader.getMinKMBetaSector2Cut();
	    max_beta = CutLoader.getMaxKMBetaSector2Cut();
	}
	else if( sector == 2 ){
	    min_beta = CutLoader.getMinKMBetaSector3Cut();
	    max_beta = CutLoader.getMaxKMBetaSector3Cut();
	}
	else if( sector == 3 ){
	    min_beta = CutLoader.getMinKMBetaSector4Cut();
	    max_beta = CutLoader.getMaxKMBetaSector4Cut();
	}
	else if( sector == 4 ){
	    min_beta = CutLoader.getMinKMBetaSector5Cut();
	    max_beta = CutLoader.getMaxKMBetaSector5Cut();
	}
	else if( sector == 5 ){
	    min_beta = CutLoader.getMinKMBetaSector6Cut();
	    max_beta = CutLoader.getMaxKMBetaSector6Cut();
	}
    ...
    public static double kaonMinusBetaConfLevel( double beta_meas, double beta_true, double p, int sector){
    ...
    //public static double KaonMinusVertexMLE( double beta ){
    ///////////////////////////////////////////
    //CHANGE THE PARAMETERS HERE FOR VERTEX MLE
    ///double likelihood = (1.0/ Math.sqrt(2.0*3.14159265358 * sigma*sigma) ) * Math.exp( -0.5 * Math.pow( (beta_pr - pr_beta_mntm )/sigma, 2 ));
    //return likelihood;	
